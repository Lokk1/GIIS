...
public class PaintPanel extends JPanel {
	...
	private int inputRadiusDialog(){
		...
		String tempValue = JOptionPane.showInputDialog(paintPanel, "Input radius");
		
		if( tempValue != null)
			
			if(!tempValue.isEmpty())
				return radius = Integer.parseInt(tempValue);
			else JOptionPane.showMessageDialog(paintPanel, "Will be selected by default");
	...
	private Point hiperbolaInputDialog(){
	    ...
	    if(a == 20 && b == 20)
	    	JOptionPane.showMessageDialog(paintPanel, "Will be selected by default");
	    if(a == 20 && b != 20)
	    	JOptionPane.showMessageDialog(paintPanel, "Will be selected by default for \"a\"");
	    if(a != 20 && b == 20)
	    	JOptionPane.showMessageDialog(paintPanel, "Will be selected by default for \"b\"");
	...
	private void clickedFirstPoint(MouseEvent e){
		...
		for (int i = 0; wProgress < width; i++) {
			...
			for (int j = 0; hProgress < height; j++) {
				if (j == 0) {
					Rectangle rectangle = new Rectangle(wProgress, j, step, step);
					if (rectangle.contains(e.getX(), e.getY()) && mainFrame.getAlgorithmType() != AlgorithmType.CIRCLE
							&& mainFrame.getAlgorithmType() != AlgorithmType.HYPERBOLA) {
						p1 = new Point(e.getX() / step, e.getY()/ step);
						mainFrame.setFirstPointCord("(" + e.getX()/ step + ", " + e.getY() / step + ")");
						clickedCells.add(new Cell(p1, Color.LIGHT_GRAY));
					}
					else if (rectangle.contains(e.getX(), e.getY()) && mainFrame.getAlgorithmType() == AlgorithmType.CIRCLE 
							&& mainFrame.getAlgorithmType() != AlgorithmType.HYPERBOLA) {
						p1 = new Point(e.getX() / step, e.getY() / step);
						mainFrame.setFirstPointCord("(" + e.getX() / step + ", " + e.getY() / step + ")");
						clickedCells.add(new Cell(p1, Color.LIGHT_GRAY));
					...
					else if (rectangle.contains(e.getX(), e.getY()) && mainFrame.getAlgorithmType() == AlgorithmType.HYPERBOLA 
					...
					Rectangle rectangle = new Rectangle(wProgress, hProgress, step, step);
					if (rectangle.contains(e.getX(), e.getY()) && mainFrame.getAlgorithmType() != AlgorithmType.CIRCLE
							&& mainFrame.getAlgorithmType() != AlgorithmType.HYPERBOLA) {
						p1 = new Point(e.getX() / step, e.getY()/ step);
						mainFrame.setFirstPointCord("(" + e.getX()/ step + ", " + e.getY() / step + ")");
					...
					if (rectangle.contains(e.getX(), e.getY()) && mainFrame.getAlgorithmType() == AlgorithmType.HYPERBOLA 
							&& mainFrame.getAlgorithmType() != AlgorithmType.CIRCLE) {
						p1 = new Point(e.getX() / step, e.getY() / step);
						mainFrame.setFirstPointCord("(" + e.getX() / step + ", " + e.getY() / step + ")");
						clickedCells.add(new Cell(p1, Color.LIGHT_GRAY));
			...
			for (int j = 0; hProgress < height; j++) {
				if (j == 0) {
					...
					if (rectangle.contains(e.getX(), e.getY())) {
						...
						mainFrame.setSecondPoint("(" + e.getX() / step + ", " + e.getY() / step + ")");
